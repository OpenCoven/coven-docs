...
Every API call below is also runnable inline: press **Run** to execute the step against a simulated daemon with real cursor and error mechanics, or flip the console at the bottom of the page to **Live** to hit your own local daemon.
...
<ApiRequest method="GET" path="/api/v1/health">
...
<ApiRequest method="GET" path="/api/v1/sessions">
...
<ApiRequest
  ...
  params={[
    { name: 'projectRoot', in: 'body', default: '/Users/you/code/your-repo', required: true },
    { name: 'cwd', in: 'body', default: '/Users/you/code/your-repo' },
    { name: 'harness', in: 'body', default: 'codex', required: true },
    { name: 'prompt', in: 'body', default: 'explain this repo in 5 bullets', required: true },
    { name: 'title', in: 'body', default: 'API launch smoke test' },
  ]}
...
<ApiRequest
  method="GET"
  path="/api/v1/events?sessionId=$SESSION_ID&afterSeq=$AFTER_SEQ&limit=100"
  params={[
    { name: 'SESSION_ID', in: 'query', required: true },
    { name: 'AFTER_SEQ', in: 'query', default: '0' },
  ]}
  capture={{ AFTER_SEQ: 'nextCursor.afterSeq' }}
>
...
<ApiRequest method="GET" path="/api/v1/sessions/does-not-exist">
