---
title: "Glossary"
description: "Key terms and definitions used throughout Coven."
---

title: "Coven glossary"
summary: "Definitions for Coven terms: ACP, API version, archive, capability, CastCodes, Cast Agent, Cast Codes, client, harness, project root, ritual, sacrifice, session, and summon."
read_when:
  - Looking up Coven terminology
  - Aligning docs, CLI copy, and client labels
description: "Definitions for Coven terms: ACP, API version, archive, capability, CastCodes, Cast Agent, Cast Codes, client, harness, project root, ritual, sacrifice, session, and summon."
...
The Coven-native substrate manager and AI agent backend inside CastCodes. Cast Agent can collect workspace context, talk to Coven Gateway, and surface local Coven sessions, but it is not the daemon authority boundary.

## Cast Codes

The portable intent grammar for familiar switching, delegation, broadcast, status, resume, and handoff requests. Cast Codes express intent; daemon validation still owns sensitive runtime decisions.

## CastCodes

The first-party OpenCoven workstation and cockpit app for visible agent work, panes, files, diagnostics, sessions, review, and merge flow.

## Client

Any process or UI that talks to the Coven daemon, including the CLI/TUI, CastCodes, Cast Agent, scripts, or the OpenClaw plugin.
...
The broader ecosystem and organization around Coven, CastCodes, Cast Agent, Cast Codes, and related integrations.
