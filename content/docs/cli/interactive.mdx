...
If the `coven-code` binary is on `PATH`, interactive `coven` and `coven tui` invocations exec into [Coven Code](/docs/coven-code), the full coding TUI, instead of the legacy shell. Set `COVEN_LEGACY_TUI=1` to opt out and keep the shell described on this page.
