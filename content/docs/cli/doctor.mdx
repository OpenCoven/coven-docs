...
Doctor can also print an npm-prefix advisory (outside `Installs:`) when `npm install -g` writes to a shadowed copy. In that case, the documented upgrade command never touches the binary your shell runs. Use the prefix-explicit command Doctor prints until the extra copies are gone:
