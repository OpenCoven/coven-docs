...
When several `coven` executables are on `PATH`, the first entry wins and silently answers
every command — an upgrade can appear to do nothing because an older binary is still
running. `coven doctor` reports installs before every other check. The prose report prints
each path:
...
The JSON report from `coven doctor --json` is deliberately path-free: check id
`install:conflicts` reports `"<N> coven executables on PATH"` with the hint
"run `coven doctor` for the paths; the first entry wins, so remove the others or reorder
PATH", and reports `"one coven on PATH"` when the install is unambiguous. The check warns
rather than fails, so it never changes the exit code on its own. After removing or
reordering installs, re-check with `coven --version`.
