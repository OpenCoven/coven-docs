...
Machines that have been through more than one install route, or more than one Node version, usually carry more than one `coven`. Uninstalling only the copy you remember leaves the others answering. Ask Doctor before removing anything; it lists every copy on `PATH`, how each was installed, and the command that removes it. Until the CLI release that includes OpenCoven/coven#1124, Doctor still prints the older path-only list; treat the block below as the forthcoming format:
