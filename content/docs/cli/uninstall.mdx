...
Machines that have been through more than one install route, or more than one Node version, usually carry more than one `coven`. Uninstalling only the copy you remember leaves the others answering. Ask Doctor before removing anything. In the CLI release that includes OpenCoven/coven#1124, it lists every copy on `PATH`, how each was installed, and the command that removes it. Until then, Doctor still prints the older path-only list; treat the block below as the forthcoming format:
...
Copies in a Node version that is not currently active do not appear on `PATH`; switch to that version (`nvm use <version>`), run `npm prefix -g`, and use `npm uninstall -g --prefix <that-prefix> @opencoven/cli` there too if you want them gone.
