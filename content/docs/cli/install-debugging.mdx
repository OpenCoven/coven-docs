...
`doctor` reports store, project, daemon, socket, and harness readiness, and warns when several `coven` installs are shadowing each other on `PATH`. If `coven doctor` fails because the command is not found, work through the sections below.
...
Multiple entries mean more than one install is on your search path. Remove the stale copy or move the intended install earlier on `PATH` before debugging anything else.

`coven doctor` performs this check itself: when more than one install resolves, its prose report opens with an `Installs:` section that marks the active copy and each shadowed one, and the first entry wins.
