...
On macOS and Linux the global bin directory is `<prefix>/bin`; on Windows it is the prefix itself. (`npm bin -g` printed it directly, but the command was removed in npm 9.)
