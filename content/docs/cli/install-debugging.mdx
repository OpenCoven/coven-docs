...
`coven doctor` reports the same condition directly: an `Installs:` section in the prose output and an `install:conflicts` warning in the `--json` report. The JSON keeps paths out, so run plain `coven doctor` for the concrete paths to remove.
