...
| A user-set npm prefix (`npm config set prefix`, `NPM_CONFIG_PREFIX`, a `~/.local` prefix) alongside a version manager | Two prefixes, both with `bin` on `PATH`. Whichever is earlier on `PATH` wins, while `npm install -g` keeps writing to the active Node's prefix. | macOS, Linux, WSL2 |
