...
| Routine automations | Stores durable routine definitions, occurrence plans, and a run ledger in the same SQLite store, and runs a plan/recover/claim tick on a 60-second scheduler thread; tick failures land in the recovery log. |
...
| `$COVEN_HOME/coven.sqlite3` | SQLite session ledger, append-only event log, and automation routines (definitions, occurrences, runs). |
