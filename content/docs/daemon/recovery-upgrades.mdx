...
summary: "How to handle stale local IPC, orphaned sessions, restart recovery, and safe daemon upgrades."
description: "Daemon recovery and upgrade guide for Coven operators: stale local endpoints, orphaned PTYs, session ledger recovery, live-session checks, and safe upgrade flow."
read_when:
...
On Unix-like hosts, if clients cannot connect but `$COVEN_HOME/coven.sock`
exists, the socket may be stale. On Windows, use `coven daemon status` to
inspect the daemon-reported named pipe instead of looking for a socket file.
