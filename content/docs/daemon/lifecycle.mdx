...
description: "Lifecycle reference for the Coven daemon: start, status, restart, stop, local IPC binding, ledger recovery, and live-session behavior."
read_when:
...
`status` is the safest first command in scripts and support flows. It tells you
whether the background process exists, which local IPC endpoint it owns, and
which API contract it is serving.

## Start

`coven daemon start` creates `$COVEN_HOME` if needed, opens the SQLite store,
binds the platform local IPC endpoint (`$COVEN_HOME/coven.sock` on Unix-like
hosts or an owner-only named pipe on Windows), writes
`$COVEN_HOME/daemon.json`, and begins serving `/api/v1`.
...
If local IPC metadata is stale after a crash, use
[Recovery and upgrades](/docs/daemon/recovery-upgrades) before deleting
anything by hand.
...
| `socket` | Legacy field name containing the active Unix socket or Windows named-pipe endpoint. |
...
When building a client, prefer the HTTP [health handshake](/docs/daemon/socket-api#health-handshake) over scraping CLI status text.
...
Restart should drain the old listener, release the previous local IPC endpoint,
reopen the store under the selected state directory, and serve the same
`/api/v1` contract again.
...
Whichever supervisor you use, keep single-instance discipline: the daemon
refuses to take over a healthy local IPC endpoint and holds a serve lock, so
let one supervisor own the process rather than mixing `coven daemon start`
with a service unit.
...
- [Session management](/docs/cli/sessions)
