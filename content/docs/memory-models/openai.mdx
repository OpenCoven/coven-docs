...
| Model selection and provider configuration | Owned by Codex; Coven does not pass or override models. |

Choosing which OpenAI model Codex uses is Codex configuration, not a `coven run` flag. Coven treats the harness as a black box that writes output.
