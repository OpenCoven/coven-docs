...
In OpenCoven's local layout, a familiar's durable notes live as files under `~/.coven/memory/<familiar>/`. `coven memory` lists them (with `--json` for scripts), showing each note's familiar, title, and last update.
