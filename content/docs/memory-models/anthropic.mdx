...
| Model selection and provider configuration | Owned by Claude Code; Coven does not pass or override models. |

Choosing which Anthropic model Claude Code uses is Claude Code configuration, not a `coven run` flag. Coven treats the harness as a black box that writes output.
