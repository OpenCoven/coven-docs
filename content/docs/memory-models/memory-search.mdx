...
Results are scored chunks with their source paths and owning familiar. Scoping with `--familiar` restricts the search to that familiar's ingested documents via an id allowlist. See [Semantic memory](/docs/memory-models/semantic-memory) for the full stack and ingest workflow.
