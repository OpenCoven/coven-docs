...
`~/.coven` belongs to the Coven daemon and is shared with the CLI, Cave, and other clients. Delete it only when you intend to erase shared local Coven sessions and events. See [CLI Uninstall -> Local state](/docs/cli/uninstall#local-state).
</Callout>
