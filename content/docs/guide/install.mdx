...
<DocsDataTable
  ...
  rows={[
    { path: 'npm wrapper `@opencoven/cli`', status: 'Supported', useWhen: 'You want the fastest public install on a published platform package.', verify: '`npx @opencoven/cli doctor` for one-off checks; `coven doctor` after global install' },
...
Move local Coven state aside when you want a clean test home without immediately deleting session history:

```sh
mv ~/.coven ~/.coven.backup.$(date +%Y%m%d-%H%M%S)
```

After you confirm the backup is no longer needed, remove it using your normal trash/recycle-bin workflow. Do not delete `~/.coven` as a routine update step.
