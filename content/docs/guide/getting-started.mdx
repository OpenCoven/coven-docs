...
This is only a preflight. It does not replace a normal install for the daemon and session steps below because those commands expect `coven` on `PATH`. Complete one of the install routes in [Install Coven](/docs/guide/install), then come back here and run `coven doctor`.
