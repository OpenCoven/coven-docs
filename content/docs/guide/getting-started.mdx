...
<DocsDataTable
  ...
  rows={[
    ...
    { check: 'Daemon/socket status', group: 'Daemon', output: 'Shows whether the daemon is running, stale, or stopped and which socket path clients should use.' },
...
- [Tinkerer's next 30 minutes](/docs/guide/tinkerers-next-30-minutes) for the daemon API, JSON output, events, and fake-harness smoke path.
- [Troubleshooting](/docs/reference/troubleshooting) for `coven pc`, stale sockets, harness setup, and recovery paths.
