...
The daemon is one same-user process per host. It owns live PTYs and the SQLite-backed session ledger, then serves the supported `/api/v1` contract over a Unix socket. The CLI and other local clients ask it to act; they do not become the enforcement boundary.
