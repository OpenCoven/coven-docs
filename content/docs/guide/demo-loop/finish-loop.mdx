...
| **Archive** | The session record + event log stay for later replay. | Yes (`coven summon` restores and replays it). |
| **Sacrifice** | The session record and event log are deleted. | No. |
...
<Steps>
  ...
  <Step>
    ...
    Keep the session record and event log so you can replay the work later. Useful when you don't want to ship the changes but want to remember what the agent tried. Archive refuses running sessions — stop the process first with `coven kill <session-id>` if it is still live.

    ```bash
    coven archive <session-id>
    ```

    Archived sessions appear in `coven sessions --all` and remain replayable through `coven summon <session-id>`, which restores the record and then replays its log.
  </Step>

  <Step>
    ### Sacrifice

    Delete the session record and its event log. This is the explicit "throw it all away" path. If the work lived in a per-lane worktree, remove the worktree separately (`git worktree remove <worktree-path>`).
