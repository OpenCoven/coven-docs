...
| WSL2 | Install inside the distro as Linux software. | Keep projects and `COVEN_HOME` on the Linux filesystem, not `/mnt/c`, for daemon-heavy work. |
| Raspberry Pi | Use a 64-bit OS and a source build. | Do not assume a published npm binary for Raspberry Pi. |

## macOS Apple Silicon

On macOS Apple Silicon, the published npm wrapper is the normal install route. Keep `COVEN_HOME` under your user account and add `launchd` only after the same shell can already verify Coven manually.
...
If you are on a macOS architecture that this docs set does not explicitly verify, use a source checkout instead of assuming the published wrapper applies.
...
Treat WSL2 as a Linux install inside one distro. Keep the project, harness CLIs, and `COVEN_HOME` on the Linux filesystem. Avoid `/mnt/c` for daemon-heavy work because Windows file semantics can interfere with sockets, permissions, and file watching.
