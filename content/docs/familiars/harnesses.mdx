...
description: "Connecting external coding-agent CLIs to Coven as harnesses."
---

import { DocsDataTable } from '@/components/docs-data-table';
import { Mermaid } from '@/components/mermaid';

Coven ships four built-in harness adapters — Codex, Claude Code, Coven Code, and Copilot CLI — and loads external adapters from manifest files, including the trusted `grok` and `hermes` recipes. This guide describes the current adapter shape and the bar for adding more harnesses.
...
Most adapters pass the prompt as the final command argument after any fixed prefix args; an adapter can instead declare a prompt flag (Copilot CLI passes the prompt through `--prompt` or `--interactive`).
...
<DocsDataTable
  ...
  rows={[
    ...
    { harness: 'Coven Code', id: '`coven-code`', executable: '`coven-code`', interactiveArgs: 'None', nonInteractiveArgs: '`--print`', setup: '`coven engine install`' },
    { harness: 'Copilot CLI', id: '`copilot`', executable: '`copilot`', interactiveArgs: 'Prompt via `--interactive`', nonInteractiveArgs: '`--no-color`; prompt via `--prompt`', setup: '`npm install -g @github/copilot`; `copilot login`' },
  ]}
/>
...
<DocsDataTable
  ...
  rows={[
    ...
    { name: '`coven-code`', status: 'Current', reason: 'Built-in adapter for the CLI-managed Coven Code engine.' },
    { name: '`copilot`', status: 'Current', reason: 'Built-in adapter and harness id in the daemon.' },
    { name: '`grok`', status: 'Current (trusted recipe)', reason: 'Installed with `coven adapter install grok`; loaded from a Coven-maintained manifest.' },
    { name: '`hermes`', status: 'Current (trusted recipe)', reason: 'Installed with `coven adapter install hermes`; loaded from a Coven-maintained manifest.' },
    ...
    { name: 'Custom local harness', status: 'Current via manifest', reason: 'External adapter manifests load through `COVEN_HARNESS_ADAPTER_MANIFEST` and `COVEN_HARNESS_ADAPTER_DIRS` when they meet the adapter bar.' },
    ...
    { name: 'Generic command adapter', status: 'Not supported', reason: 'Arbitrary commands need explicit policy, approval, and audit behavior before they can be safe.' },
  ]}
/>
