...
summary: "How comux, OpenClaw, and other clients should talk to the Coven daemon socket API without duplicating runtime policy or authority."
read_when:
  - Integrating a UI or bridge with Coven
  - Avoiding client-side policy drift
description: "How comux, OpenClaw, and other clients should talk to the Coven daemon socket API without duplicating runtime policy or authority."
...
<DocsDataTable
  caption="comux responsibilities when acting as a Coven cockpit"
  searchPlaceholder="Filter comux responsibilities..."
  columns={[
  ...
  rows={[
    ...
    { capability: 'Read coven sessions JSON', group: 'Session discovery', dataSource: 'CLI JSON', clientBehavior: 'Use simple local discovery when daemon-level control is unnecessary.' },
