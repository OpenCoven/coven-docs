...
description: "Definition of a Coven harness and its boundary with the daemon."
read_when:
...
| Session | Coven-owned record of one harness run: its project, events, and lifecycle. |
| Client | CLI, TUI, or script that asks the daemon to act. |
...
The same discipline applies to "runtime": Coven is the local runtime around the harness; the harness is the agent process that performs the work.
